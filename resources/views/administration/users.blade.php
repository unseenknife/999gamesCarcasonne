                        ...
                        <table id="myTable" style="width:100%;">
                              <thead role="rowgroup">
                                <tr role="row">
                                    ...
                                    <th role="columnheader" class="hidden-mobile-row">Email</th>
                                    <th role="columnheader" class="hidden-mobile-row">Rol</th>
                                    <th role="columnheader" class="hidden-mobile-row">Telefoon</th>
                                    
                                </tr>
                              </thead>
                              <tbody role="rowgroup">
                                @foreach($users as $user)
                                <tr role="row" onclick="DoNav('{{ route('user-management')}}/{{$user->id}}');">
                                    <td role="cell">{{$user->f_name}} {{$user->l_name}}</td>
                                    
                                    <td role="cell" class="hidden-mobile-row">{{$user->email}}</td>
                                    <td role="cell" class="hidden-mobile-row">{{$user->roles->name}}</td>
                                    <td role="cell" class="hidden-mobile-row">{{$user->phone_nr}}</td>
